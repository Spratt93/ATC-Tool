...
class SideView extends Visualisations {
    ...
    protected void setArrows(Obstacle obstacle) {
        ...
        if (!isReversed) {
            obstacleXPos = obstacle.getdThreshold1() + currentLogical.getThresholdDisplacement();
        } else {
            obstacleXPos = obstacle.getdThreshold2() - currentLogical.getThresholdDisplacement();
        }
        ...
        if (!isReversed) {
            if (currentLogical.getNewStopway() != 0) {
                setArrowVisibility("Stopway", true);
                changeArrow("Stopway", oldTORA + runwayOffset/scale, -runwayOffset/scale, getHeight()*0.15, "Stopway: " + currentLogical.getNewStopway() + "m");
            }
            if (currentLogical.getNewClearway() != 0) {
                setArrowVisibility("Clearway", true);
                changeArrow("Clearway", oldTORA + runwayOffset/scale, -runwayOffset/scale, getHeight()*0.2, "Clearway: " + currentLogical.getNewClearway() + "m");
            }
        } else {
            if (currentLogical.getNewStopway() != 0) {
                setArrowVisibility("Stopway", true);
                changeArrow("Stopway", -runwayOffset/scale, runwayOffset/scale, getHeight()*0.15, "Stopway: " + currentLogical.getNewStopway() + "m");
            }
            if (currentLogical.getNewClearway() != 0) {
                setArrowVisibility("Clearway", true);
                changeArrow("Clearway", -runwayOffset/scale, runwayOffset/scale, getHeight()*0.2, "Clearway: " + currentLogical.getNewClearway() + "m");
            }
        }
